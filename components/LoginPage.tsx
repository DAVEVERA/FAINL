...
export const LoginPage: FC<LoginPageProps> = ({ onLoginSuccess }) => {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) throw error;
      setMessage({ type: 'success', text: 'Neural Link gestuurd! Controleer je e-mail om verder te gaan.' });
    } catch (err: any) {
      setMessage({ type: 'error', text: err.message });
    } finally {
      setIsLoading(false);
    ...
    <div className="min-h-[80vh] flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md bg-white dark:bg-zinc-900 border-4 border-black dark:border-white/20 p-8 md:p-12 shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] dark:shadow-[16px_16px_0px_1px_rgba(255,255,255,0.1)]">
        <div className="flex flex-col items-center text-center mb-10">
          ...
          <h2 className="text-4xl font-black uppercase tracking-tighter mb-2 text-black dark:text-white">FAINL</h2>
          <p className="text-black/40 dark:text-white/40 font-bold uppercase text-[10px] tracking-widest text-balance leading-relaxed">Identiteitsverificatie vereist om toegang te krijgen tot de beveiligde sessiegeschiedenis</p>
        </div>
