    ...
    >
      ...
      <div className="flex flex-col border-b-2 md:border-b-4 border-black dark:border-white/20 bg-white dark:bg-zinc-900/50">
        ...
        <div className="p-3 md:p-4 pb-2 flex items-center gap-2 md:gap-3">
          <div
            className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center ${member.color} shadow-sm border-2 border-black dark:border-white/40 shrink-0`}
          >
            <img
              src={member.avatar}
              alt={member.name}
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-black dark:text-white text-xs md:text-sm uppercase tracking-wide truncate">
              {member.name}
            </h3>
      ...
      {/* Content Area */}
      <div
        className={`flex-1 p-3 md:p-4 pr-2 md:pr-3 overflow-y-auto min-h-[150px] md:min-h-[200px] max-h-[300px] md:max-h-[400px] bg-white dark:bg-zinc-900/30 text-xs md:text-sm text-black dark:text-white border-t-2 border-black/5 dark:border-white/5`}
      >
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-full gap-2 text-black/50 dark:text-white/40">
            <span className="animate-pulse font-mono uppercase text-[10px] md:text-xs">
              Processing...
            </span>
          </div>
        ) : response ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-black/30 dark:text-white/20">
            ...
            <span className="font-mono text-[10px] md:text-xs uppercase">
              Standby
            </span>
          </div>
        )}
