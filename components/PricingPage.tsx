...
export const PricingPage: FC<PricingPageProps> = ({
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-6 py-10 md:py-16 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="text-center mb-10 md:mb-16">
        <h1 className="text-3xl md:text-5xl font-black uppercase tracking-tighter mb-4 text-black dark:text-white leading-none">
      ...
      <div className="mb-20">
        <div className="flex items-center gap-4 mb-10 justify-center sm:justify-start">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-black uppercase tracking-tighter text-black dark:text-white">{t.pricing.standardAccess}</h2>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-black/40 dark:text-white/40">Multi-AI Analyse Bundels</p>
          </div>
        </div>
          ...
          {PRICING.CREDITS.map((pkg, idx) => (
            <div key={idx} className="card-fainl">
              ...
              <button onClick={() => handlePurchase(pkg.count)} className="card__cta">
                Koop {pkg.count} {pkg.count === 1 ? 'credit' : 'credits'}
              </button>
            </div>
          ))}
        </div>
      ...
      <div>
        <div className="flex items-center gap-4 mb-10 justify-center sm:justify-start">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-black uppercase tracking-tighter text-black dark:text-white">Abonnementen</h2>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-black/40 dark:text-white/40">Maandelijkse neurale capaciteit</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 justify-center sm:justify-start">
          {PRICING.SUBSCRIPTIONS.map((pkg, idx) => (
            <div key={idx} className="card-fainl !border-[#FDC700] hover:!shadow-[12px_12px_0_0_#FDC700]">
              ...
              <p className="card__subtitle">Onbeperkt archief.</p>

              <ul className="card__lists">
                <li className="card__list"><CheckIcon /> <span>{pkg.count} Vragen p/m</span></li>
                <li className="card__list"><CheckIcon /> <span>Hogere prioriteit</span></li>
                <li className="card__list"><CheckIcon /> <span>Directe beraadslaging</span></li>
                <li className="card__list"><CheckIcon /> <span>Multi AI inzet</span></li>
                <li className="card__list"><CheckIcon /> <span>Exclusieve modellen</span></li>
              </ul>
